...
二叉树是一种重要的数据结构，每个节点最多有两个子节点（左子节点和右子节点）。二叉树存在一些基本概念：
...
class Solution:
    def maxScore(self, nums: List[int]) -> int:
                ...
                for j in range(2 * n):
                    ...
                    if mask & (1 << j):
                        for k in range(j + 1, 2 * n):
                            ...
                            if mask & (1 << k):
                                # 计算选择 j 和 k 这对数字的得分
                                # 1. 计算不包含 j 和k的 mask 状态：mask ^ (1 << j) ^ (1 << k)
                                # 2. 获取前 i-1 对数字的得分：dp[i-1][mask_without_j_and_k]
        ...
        # 返回结果：选择了 n 对数字且所有数字都被选中的最大得分
        # (1 << (2 * n)) - 1 表示所有数字都被选中的 mask 状态
        return dp[n][(1 << (2 * n)) - 1]
```
